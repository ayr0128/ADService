...
{
    public partial class FormCreateGroup : Form, IFormEditable
    {
        /// <summary>
        /// 此介面固定的呼叫方法
        /// </summary>
        private const string METHOD = Methods.M_CREATEGROUP;
        /// <summary>
        /// 按下確定後透過此功能呼叫修改
        /// </summary>
        ...
        /// </summary>
        private readonly CreateGroup requestClass;
        ...
        public FormCreateGroup(in ADAgreement agreement, ADInvokeCondition invokeCondition)
        {
            InitializeComponent();
            ...
            requestClass = typeof(CreateGroup).Name == receivedDescription && isEditable ? new CreateGroup() : null;

            // 清空
            InputBoxGroupName.Text = string.Empty;
        ...
        private void TextBoxGroupName_TextChanged(object sender, System.EventArgs e)
        {
            ...
            #region 驗證新需求是否可用
            // 轉換發送者: 此處會取得輸入完成的字串
            TextBox inputBoxGroupName = sender as TextBox;
            // 取得新的群組名稱
            CreateGroup protocolAuthenicate = new CreateGroup() { Name = inputBoxGroupName.Text };
            // 轉換成協定
            JToken protocol = JToken.FromObject(protocolAuthenicate);
            // 發出協定進行驗證
            ...
            else
            {
                ...
                // 改回舊名稱
                inputBoxGroupName.Text = requestClass.Name;
            }
        }
